...
Texture::Texture(SDL_Rect rect, const Window &wnd)
    : Drawable(rect, wnd), texture(nullptr) {}
...
Texture::Texture(const std::string &file, SDL_Rect rect, const Window &wnd)
  ...
  if (!texture)
    throw std::runtime_error(
        std::string("Error: Failed to load file:") + file +
        std::string("\nsuggestion: make sure SDL2 has been initiallized and "
                    "the file path leads to an existing file."));
}

Texture::Texture(const Texture &other)
    : Drawable(static_cast<Drawable>(other)) {
  SDL_Point size;
  Uint32 format;
  SDL_QueryTexture(other.texture, &format, 0, &size.x, &size.y);
  texture = SDL_CreateTexture(window->Renderer, format,
...
Texture::Texture(Texture &&other) : Drawable(other), texture(other.texture) {
...
Vec2 Texture::Rotate(float angle, SDL_RendererFlip flip, bool resize) {
  ...
  if (angle != 0 && size.x != size.y) {
    ...
    W = size.x * abs(cos(radians)) +
        size.y * abs(sin(angle * 3.14159265 / 180));
    H = size.x * abs(sin(radians)) +
        size.y * abs(cos(angle * 3.14159265 / 180));
  }

  SDL_Texture *flipped = SDL_CreateTexture(window->Renderer, format,
                                           SDL_TEXTUREACCESS_TARGET, W, H);
  SDL_SetRenderTarget(window->Renderer, flipped);
  SDL_Rect TexRect = {(W - size.x) / 2, (H - size.y) / 2, size.x, size.y};
  SDL_RenderCopyEx(window->Renderer, texture, nullptr, &TexRect, angle, 0,
                   flip);
  ...
  SDL_DestroyTexture(texture);
  texture = flipped;
  // resize the texture rect such that the rotated texture will look the same
  // size and wont shrink in order to fit in the rect.
  if (!resize) {
    rect.x -= (W - size.x) / 2;
    rect.y -= (H - size.y) / 2;
