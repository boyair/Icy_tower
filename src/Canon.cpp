...
void Canon::Change_Power(float power) { this->power = power; }
...
bool Canon::InTrajectory(SDL_Rect rect) {
  ...
  Vec2 ballvel = direction == SDL_FLIP_HORIZONTAL
                     ? Vec2{power, power * -0.5f}
                     : Vec2{power * -1, power * -0.5f};
  int distance_between_centers =
      std::max(((rect.x) - (ball.hitbox.x + ball.hitbox.w)),
               ((rect.x + rect.w) - (ball.hitbox.x)));
  ...
  float YCenterOnXcollision =
      (ball.position.y + ball.hitbox.h / 2.0f) +
      time_to_reach_rect_center * ballvel.y +
      (std::pow(time_to_reach_rect_center, 2) * gravity / 2.0f);
  return rect.y < YCenterOnXcollision && rect.y + rect.h > YCenterOnXcollision;
}

void Canon::SetFlip(SDL_RendererFlip flip) { direction = flip; }
