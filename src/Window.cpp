...
Window::Window(const char *title, const SDL_Rect &rect, Uint32 flags)
  ...
  wnd = SDL_CreateWindow(title, rect.x, rect.y, rect.w, rect.h,
...
SDL_Point Window::GetRelativePoint(SDL_Point original) const {
...
SDL_Rect Window::GetRelativeRect(SDL_Rect original) const {
  original.x -= CameraView.x;
  original.y -= CameraView.y;
  original.x *= static_cast<float>(width) / CameraView.w;
  original.y *= static_cast<float>(height) / CameraView.h;
...
void Window::Show() { SDL_RenderPresent(Renderer); }

void Window::SetBGColor(unsigned char red, unsigned char green,
                        unsigned char blue) {
...
SDL_Rect Window::ScaleRect(SDL_Rect rect) const {
  ...
  return {(int)(rect.x * ((float)width / CameraView.w)),
          (int)(rect.y * ((float)height / CameraView.h)),
          (int)(rect.w * ((float)width / CameraView.w)),
          (int)(rect.h * ((float)height / CameraView.h))};
}
