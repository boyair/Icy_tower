...
Text::Text(SDL_Color color, Window &window, SDL_Rect rect)
    : color(color), font(TTF_OpenFont("../fonts/font.ttf", 100)),
      texture(rect, window) {}

Text::Text(const std::string &text, const std::string &font_file_path,
           Window &window, SDL_Rect rect)
    : text(text), color({255, 255, 255, 255}),
      font(TTF_OpenFont(font_file_path.c_str(), 100)), texture(rect, window) {
  HandleFontLoadingError(font_file_path);
  RecreateTexture();
}

Text::Text(SDL_Color color, const std::string &font_file_path, Window &window,
           SDL_Rect rect)
    : color(color), font(TTF_OpenFont("../fonts/font.ttf", 100)),
      texture(rect, window)
...
Text::Text(const std::string &text, Window &window, SDL_Rect rect)
    : text(text), color({255, 255, 255, 255}),
      font(TTF_OpenFont("../fonts/font.ttf", 100)), texture(rect, window) {
  HandleFontLoadingError("../fonts/font.ttf");
  RecreateTexture();
}

Text::Text(const std::string &text, SDL_Color color, Window &window,
           SDL_Rect rect)
    : text(text), color(color), font(TTF_OpenFont("../fonts/font.ttf", 100)),
      texture(rect, window)
...
Text::Text(const std::string &text, SDL_Color color,
           const std::string &font_file_path, Window &window, SDL_Rect rect)
    : text(text), color(color), font(TTF_OpenFont(font_file_path.c_str(), 100)),
      texture(rect, window)
...
void Text::Draw() { texture.DrawOnWindow(true); }
